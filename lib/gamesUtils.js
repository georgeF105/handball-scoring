...
export function initializeGame (gameKey, game, homeTeam, awayTeam) {
  getFullGame(gameKey)
    .then(res => {
      console.log('game here', res)
      updateGame(gameKey, res)
    })
    .catch(err => {
      console.error(err)
    })
...
function getFullGame(gameKey) {
  ...
  return getGame(gameKey)
    .then(res => {
      ...
      return Promise.all([getFullTeam(game.home_team),getFullTeam(game.away_team)])
    })
    .then(res => {
      game.home_team = Object.assign({}, res[0], {key: game.home_team})
      game.away_team = Object.assign({}, res[1], {key: game.away_team})
      return game
    })
    .catch(err => {
      console.error(err)
...
function getFullTeam(teamKey) {
  ...
  return getTeam(teamKey)
    ...
    .then(res => {
      ...
      //   team.players[key] = Object.assign({}, res[index], team.players)
      // })
      return team
    })
    .catch(err => {
